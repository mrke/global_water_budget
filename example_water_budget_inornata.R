...
Et.reported<-0.0074  #ml/hr, Total water loss rate
...
mass<-13.37             #g, body mass
SA<-56.33              #cm2, surface area of animal, in not known use below equation from Benedict
#SA=10*mass^(2/3)
VO2<-0.75            #mlO2/h, Metabolic rate, if unknown use regression equation below (Anders Pough 85) 
...
cross.animal<-3.14*(1.32/2)^2 #cm2, ave cross sectional area of animal, I am using ave of head width/height as diameter
...
SAeye<-2*3.14*(0.165/2)^2      #cm2, surface area of eye, from Rob's manuscript it is 1.54cm2, seems too big so I used ratio of eye to SVL from photo (0.022) with ave SVL of our adult lizards (75.06mm) to get eye diameter
...
longlat<-c(145,-37) # type a long/lat here in decimal degrees, used if option 0 is chosen above
...
TMAXPR<-38.1 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Licht 1966 thermal gradient
TMINPR<-24.6 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # Kearney Obs (PhD field trip)
...
ctmax<-42.7  # degrees C, critical thermal maximum (animal will die if ctkill = 1 and this threshold is exceeded)
ctmin<-8.6 # degrees C, critical thermal minimum (used by program to determine depth selected when inactive and burrowing)
TPREF<-28.8 # preferred body temperature (animal will attempt to regulate as close to this value as possible) (mean 31.9, range 29.4-34.3, Bennett, A.F. & John-Alder, H. (1986) Thermal Relations of Some Australian Skinks (Sauria: Scincidae). Copeia, 1986, 57-64.), mode in Pamula Fig. 3.14 around 33.5
#Pwet from actual Ec ranges from 0.1 to 1.2 in my database
skinwet<-0.31 # %, percentage of total surface area acting like a free water surface for evaporation 
...
amass<-13.6 # g, mass of animal (used if the 'monthly' option is checked and DEB model is thus off)
MR_1<-10^-3.552
MR_2<-0.304
MR_3<-0.039
