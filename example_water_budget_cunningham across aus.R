...
nicheout<-tryCatch(NicheMapR(niche), error = function() next) 
setwd(curdir)
...
if (p==1){
JanEtmin[w]<-max(masbal$H2OEvap_g[which(masbal$JULDAY==15)])
JulyEtmin[w]<-max(masbal$H2OEvap_g[which(masbal$JULDAY==196)])
}
if (p==2){
JanEtcunn[w]<-max(masbal$H2OEvap_g[which(masbal$JULDAY==15)])
JulyEtcunn[w]<-max(masbal$H2OEvap_g[which(masbal$JULDAY==196)])
}
if (p==3){
JanEtmax[w]<-max(masbal$H2OEvap_g[which(masbal$JULDAY==15)])
JulyEtmax[w]<-max(masbal$H2OEvap_g[which(masbal$JULDAY==196)])
}
