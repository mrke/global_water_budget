...
library(lattice)
    ...
    skinwet<-pwet[p] # %, percentage of total surface area acting like a free water surface for evaporation 
    ...
    if (p==1){
      JanEtmin[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==15)])
      JulyEtmin[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==196)])
    }
    if (p==2){
      JanEtcunn[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==15)])
      JulyEtcunn[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==196)])
    }
    if (p==3){
      JanEtmax[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==15)])
      JulyEtmax[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==196)])
    }
    points(x=sites[w,1],y=sites[w,2],pch=19,col="blue",cex=(.2+JanEtcunn[w]))
    environ2<-environ
    ...
    points(x=sites[w,1]+1,y=sites[w,2]-1,pch=19,col="dark grey",cex=JanEtcunn[w]/act*15)

  }#end if over land statement
}#end w loop

# if (p==1){write.csv(masbal, file="masbal_minpwet_cunningham.csv")
#  write.csv(environ, file="environ_minpwet_cunningham.csv")} 
# if (p==3){write.csv(masbal, file="masbal_maxpwet_cunningham.csv")
#  write.csv(environ, file="environ_maxpwet_cunningham.csv")}
# if (p==2){write.csv(masbal, file="masbal_cunningham.csv")
#  write.csv(environ, file="environ_cunningham.csv")}
# }#end p loop

# gplot(JanEtmax) + geom_tile(aes(fill = value)) +
#   scale_colour_brewer(palette="Blues") +
#   coord_equal() + 
#   geom_text(aes(label=sprintf("%0.3f", value)), colour='white',size=5) + ggtitle("E. striata: Max Et of middle day in Jan - min Pwet")
# 
# gplot(JulyEtmax) + geom_tile(aes(fill = value)) +
#   scale_colour_brewer(palette="Blues") +
#   coord_equal() + 
#   geom_text(aes(label=sprintf("%0.3f", value)), colour='white',size=5) + ggtitle("E. striata: Max Et of middle day in July - min Pwet")
# 
# gplot(JulyEtstriata) + geom_tile(aes(fill = value)) +
#   scale_colour_brewer(palette="Blues") +
#   coord_equal() + 
#   geom_text(aes(label=sprintf("%0.3f", value)), colour='white',size=5) + ggtitle("E. striata: Max Et of middle day")
# 
# 
# 
# diff<-(JanEtmax - JanEtmin)/JanEtmin*100
# gplot(diff) + geom_tile(aes(fill = value)) +
#   scale_colour_brewer(palette="Blues") +
#   coord_equal() + 
#   geom_text(aes(label=sprintf("%0.3f", value)), colour='white',size=5) + ggtitle("Diurnal burrowing: middle day in Jan - percent increase in Et from Pwetmin to Pwetmax")
# 
# 
# mypath<-file.path("C:","Users","Elia","Dropbox","NicheMapR results","Striata Jan.pdf")
# pdf(file=mypath, width=18,height=11,onefile=FALSE)
# gplot(diff) + geom_tile(aes(fill = value)) +
#   scale_colour_brewer(palette="Blues") +
#   coord_equal() + 
#   geom_text(aes(label=sprintf("%0.3f", value)), colour='white',size=5) + ggtitle("Diurnal burrowing: middle day in Jan - percent increase in Et from Pwetmin to Pwetmax")
