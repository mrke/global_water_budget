...
JulyEtsumperact_max<-r
JanEtsum_striata<-r
JulyEtsum_striata<-r
JanEtact_striata<-r
JulyEtact_striata<-r
JanEtsumperact_striata<-r
JulyEtsumperact_striata<-r
...
JanEtsum_striata[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==15)])/amass
JulyEtsum_striata[w]<-sum(masbal$H2OEvap_g[which(masbal$JULDAY==196)])/amass

environ2<-environ
environ2$ACT[environ2$ACT==2]<-1
JanEtact_striata[w]<-sum(environ2$ACT[which(environ2$JULDAY==15)])
JulyEtact_striata[w]<-sum(environ2$ACT[which(environ2$JULDAY==196)])

JanEtsumperact_striata[w]<-JanEtsum_striata[w]/JanEtact_striata[w]
JulyEtsumperact_striata[w]<-JulyEtsum_striata[w]/ JulyEtact_striata[w]

   }#end if over land statement
 }#end w loop


# striata_eopen<-brick(c(JanEtsum_striata,JulyEtsum_striata,JanEtact_striata,JulyEtact_striata,JanEtsumperact_striata,JulyEtsumperact_striata))
# writeRaster(striata_eopen, filename="striata_eopen.grd", bandorder='BIL', overwrite=TRUE)
striata_rs700<-brick(c(JanEtsum_striata,JulyEtsum_striata,JanEtact_striata,JulyEtact_striata,JanEtsumperact_striata,JulyEtsumperact_striata))
writeRaster(striata_rs700, filename="striata_rs700.grd", bandorder='BIL', overwrite=TRUE)
